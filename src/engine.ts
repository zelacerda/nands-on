...
import { pinKey, type SignalState } from './simulator';
...
export class Simulator {
  ...
  /**
   * Processa a fila de eventos em ordem de tempo até esvaziar, ou até atingir o
   * teto de delta-cycles (guarda contra oscilação). Cada "tempo" é um delta-cycle:
   * todas as portas agendadas para aquele instante são avaliadas juntas.
   */
  private settle(): void {
    let steps = 0;
    for (;;) {
  ...
  /**
   * Define o valor de uma entrada (estática) e propaga. Agenda as portas que
   * consomem o net da entrada e assenta o circuito. No-op se o nó não for uma
   * entrada conhecida ou o valor não mudar.
   */
  setInput(nodeId: string, value: boolean): void {
    const net = this.inputNet.get(nodeId);
    if (!net || this.value.get(net) === value) return;
  ...
  /**
   * Avança o tempo simulado até o instante `now` (ms), processando os eventos
   * pendentes. Nesta fase não há fonte de clock — o clock por pulso/edge entra
   * na Fase 3; aqui apenas assenta o que estiver pendente.
   */
  advanceTo(_now: number): void {
    this.settle();
  }
